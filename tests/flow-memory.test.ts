...
class QueueLLM implements LLMBase {
  ...
  async generateObject() { return this.q.shift(); }
  async generateText() { return JSON.stringify(this.q.shift()); }
  async embedText() { return [0]; }
  async embedBatch() { return [[0]]; }
  streamText(): AsyncIterable<string> { async function* g(){} return g(); }
  async streamObject(): Promise<any> { return { partialStream: (async function* g(){})(), final: Promise.resolve(this.q.shift()) }; }
}

describe('Flow memory integration', () => {
  it('records flow enter/exit in state.flow_state', async () => {
    const steps = [
      { step_id: 'start', description: 'start', routes: [{ target: 'flow_entry', condition: 'go' }], available_tools: [] },
      { step_id: 'flow_entry', description: 'in flow', routes: [{ target: 'inside', condition: 'go' }], available_tools: [], flow_id: 'myflow' },
      { step_id: 'inside', description: 'inside', routes: [{ target: 'end', condition: 'exit' }], available_tools: [], flow_id: 'myflow' },
